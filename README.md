...
| _see all session options_                   | [8.3 session]          | [8.3 session]          | [8.3 session]          | [8.2 session]          |
...
Codebase volume `$APP_ROOT` (`/var/www/html`) is owned by `wodby:wodby`. Managed directories in the files volume
`$FILES_DIR` (`/mnt/files`) are owned by `www-data:www-data` with mode `2775`. The setgid bit makes new entries
inherit the `www-data` group. Default POSIX ACLs allow both `wodby` and PHP-FPM to manage newly created files and
directories, including when the creating process uses umask `0022`. Explicit restrictive modes supplied by an
application, such as `0600`, still take precedence.

Startup prepares the managed directories without recursively changing existing content. To repair an existing
shared tree, run `sudo files_chmod /mnt/files/public` (or the applicable directory under `/mnt/files`). Imports
automatically normalize owner/group access and prepare nested directories for subsequent shared writes. Imported
executable bits and permissions for other users are preserved.

Filesystems without POSIX ACL support emit a warning and retain setgid group inheritance. On those mounts,
writers must also use group-writable modes or umask `0002`; setgid alone does not grant group write access.
These changes do not grant PHP-FPM write access to the codebase or `wodby` home directory.

See https://github.com/wodby/php/issues/22 for more details.

#### Helper scripts

- `files_chmod` – to repair shared access on an existing tree, run `sudo files_chmod [FILEPATH]` (FILEPATH must be
  under `/mnt/files`). It recursively sets mode `ug=rwX,o=rX`, changes the group to `www-data`, and prepares
  directories with setgid and default ACLs. File owners are preserved.
