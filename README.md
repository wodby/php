...
Files volume `$FILES_DIR` (`/mnt/files`) owned by `www-data:www-data` with `775` mode. In case you need write access for `wodby` user to a file/dir generated by `www-data` on this volume run `sudo chmod-files-dir.sh [FILEPATH]` script (FILEPATH must be under `/mnt/files`), it will recursively change the mode to `775`.
