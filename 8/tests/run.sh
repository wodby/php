...
docker run --rm --network none --entrypoint /bin/bash -v "$PWD/workspace-initialization.sh:/tmp/workspace-initialization.sh:ro" "${IMAGE}" /tmp/workspace-initialization.sh
